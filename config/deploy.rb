...
set :domain, 'nohm.eu'
set :user, 'nohm'
set :deploy_to, '/var/www/shop_mina'
set :repository, 'git@github.com:JulianLubbers/TVA_Webshop.git'
set :branch, 'master'
...
task setup: :environment do
  ...
  #queue! %(mkdir -p "#{deploy_to}/shared/pdfs")
  #queue! %(chmod g+rx,u+rwx "#{deploy_to}/shared/pdfs")
...
task deploy: :environment do
  deploy do
    # Deploying on an friday.
    if Time.now.friday?
      which = Random.rand(0..1)
      motds = [
        %{
            ┓┏┓┏┓┃
        ┛┗┛┗┛┃＼○／
